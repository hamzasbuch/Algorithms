...
void rb_tree::inorder_output(rb_tree_node* x, int level,int* input_arr)
{ /*<<<*/
    /*
...
int rb_tree::check_black_height(rb_tree_node* x)
{
    ...
    if(x == T_nil)
    {
        ...
        left_black_height = check_black_height(x->left) + ((x->left->color == RB_BLACK) ? 1:0);
        right_black_height = check_black_height(x->right) + ((x->right->color == RB_BLACK) ? 1:0);
